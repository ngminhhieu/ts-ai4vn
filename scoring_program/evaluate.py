...
if __name__ == "__main__":
    ...
    for folder_th in range(1, num_folders+1):
        ...
        if num_files_ref > num_files_res:
            print("Missing {} files in the folder res/{}".format(num_files_ref - num_files_res, folder_th))
        elif num_files_ref < num_files_res:
            print("Exceeding {} files in the folder res/{}".format(num_files_res - num_files_ref, folder_th))
        else:
            for file_th in range(1, num_files+1):
                gt_df = pd.read_csv(os.path.join(ref_folder, "gt_{}_{}.csv".format(folder_th, file_th)), usecols=["PM2.5"])
                pred_df = pd.read_csv(os.path.join(res_folder, "res_{}_{}.csv".format(folder_th, file_th)), usecols=["PM2.5"])
                if len(pred_df) < len(gt_df):
                    print("Missing {} values in the file res_{}_{}.csv".format(len(gt_df)-len(pred_df), folder_th, file_th))
                if len(pred_df) > len(gt_df):
                    print("Exceeding values in the file res_{}_{}.csv".format(len(pred_df)-len(gt_df), folder_th, file_th))
                else:
                    gt_np[folder_th-1, file_th-1, :] = gt_df.values.flatten()
                    pred_np[folder_th-1, file_th-1, :] = pred_df.values.flatten()
